...
def writeTable(output, data):
    ...
    for row in data:
        ...
        output.write("</td>")
        # column 3 predicted Ranking
        output.write("<td style='vertical-align:top;text-align:left;' width='5%'>")
        for i in range(0, len(row["predictedRanking"])):
            output.write("%s<br>" % row["predictedRanking"][i])
        output.write("</td>")
        # column 4 text
        ...
        output.write("</td>")
        # column 5 - 8 TP FP FN TN columns
...
function refreshTable() {
	...
	for (var row = 0; row < rows.length; row++) {
		...
		if (tp.checked && cols[3].innerHTML != "" || 
			...
			cols[4].style.display = tp.checked ? 'block' : 'none';
			cols[5].style.display = fp.checked ? 'block' : 'none';
			cols[6].style.display = fn.checked ? 'block' : 'none';
			cols[7].style.display = tn.checked ? 'block' : 'none';
		}
		else {
