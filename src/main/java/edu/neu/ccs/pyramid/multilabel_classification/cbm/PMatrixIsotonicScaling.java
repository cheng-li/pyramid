...
/**
 * Created by Rainicy on 1/30/18
 *
 * run isotonic regression on pMatrix, which is used in GeneralFMeasure.
 */
public class PMatrixIsotonicScaling {
    ...
    public PMatrixIsotonicScaling(CBM cbm, MultiLabelClfDataSet dataSet) {
        this.cbm = cbm;
        ...
        total = IntStream.range(0, dataSet.getNumDataPoints()).parallel()
                .mapToObj(i -> {
                    double[] probs = cbm.predictAssignmentProbs(dataSet.getRow(i), allAssignments);
                    GeneralF1Predictor generalF1Predictor = new GeneralF1Predictor();
                    double[][] p = generalF1Predictor.getPMatrix(cbm.getNumClasses(), allAssignments,
