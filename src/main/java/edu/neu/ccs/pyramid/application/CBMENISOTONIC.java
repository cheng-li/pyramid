...
//        reportHammingPrediction(config, cbm, testSet);
    ...
    private static void reportF1Prediction(Config config, CBM cbm, MultiLabelClfDataSet dataSet, MultiLabelClfDataSet validSet) throws Exception{
        System.out.println("============================================================");
...
//        System.out.println(mlMeasures);
        ...
        // Here we do not use approximation
        MultiLabel[] finalPredictions = predictions;
        double[] setProbs = IntStream.range(0, predictions.length).parallel().
                mapToDouble(i->cbm.predictAssignmentProb(dataSet.getRow(i), finalPredictions[i])).toArray();
...
//
    ...
    private static CBM newCBM(Config config, MultiLabelClfDataSet trainSet, HyperParameters hyperParameters){
        ...
        cbm = CBM.getBuilder()
                ...
                .setSupport(DataSetUtil.gatherMultiLabels(trainSet))
                .build();
