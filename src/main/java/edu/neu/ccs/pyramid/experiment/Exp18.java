...
//
//        trainModel(config,trainDataSet,featureMappers,index, trainIdTranslator,unigramStart,unigramEnd);
//
...
//                           SingleLabelIndex index, IdTranslator trainIdTranslator, int unigramsStart,
//                           int unigramEnd) throws Exception{
...
//                Set<String> usedUnigrams = LKTBInspector.recentlyUsedFeatures(lkTreeBoost,k)
//                        .stream().filter(featureIndex -> (featureIndex>=unigramsStart && featureIndex<=unigramEnd))
//                        .map(featureMappers::getName)
//                        .collect(Collectors.toSet());
//                System.out.println("seeds added for class "+k+"("+labelTranslator.toExtLabel(k)+ ")"+": "+usedUnigrams);
//                seedsForAllClasses.get(k).addAll(usedUnigrams);
//            }
//
...
//            boolean condition3 = iteration> config.getInt("extraction.startIteration");
//
//
//
...
//                                       String[] ids) throws Exception{
//        int minDf = config.getInt("unigram.minDf");
...
//                    }
//
//                    if (source.equalsIgnoreCase("matching_score")||source.equalsIgnoreCase("matching_score")){
