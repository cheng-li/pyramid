...
//                    }
    ...
    static List<Ngram> rank(Config config, Set<Ngram> ngrams, ESIndex index,
        ...
        BufferedWriter bw = new BufferedWriter(new FileWriter(new File(config.getString("archive.folder"),"rankedFeatures.txt")));
        for (Ngram ngram: ranked){
            bw.write(ngram.toString());
            bw.newLine();
        }
